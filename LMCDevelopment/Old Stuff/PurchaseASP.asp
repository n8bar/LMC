...
Select Case sAction
	...
	Case "GetArchivedPurchaseList"	
		GetArchivedPurchaseList
...
Sub GetPurchaseList  () '----------------------------------------------------------------------------------------------------
	...
	set rs=Server.CreateObject("ADODB.Recordset")
	rs.Open SQL, REDconnstring	
	'Dim MaxPurchasing : MaxPurchasing=10

	PurchaseList=rs.GetRows()
	
	L=L&"<RecordCount>"&uBound(PurchaseList,1)&"</RecordCount><FieldCount>"&uBound(PurchaseList,2)-1&"</FieldCount>"
	For LoopNum = 0 to uBound(PurchaseList,2)-1
		L=L&"<Row"&LoopNum&">"
		For FieldNum=0 to uBound(PurchaseList,1)-1
			FVal=PurchaseList(FieldNum,LoopNum)
...
Sub RefreshNotesAndToDoSelect() '-------------------------------------------------------------------------------------------
	...
	SQL = "UPDATE PurchaseToDo SET Selected = '0'"
	set rs=Server.CreateObject("ADODB.Recordset")
	rs.Open SQL, REDconnstring
	set rs = nothing

	SQL1 = "UPDATE PurchaseNotes SET Selected = '0'"
	set rs1=Server.CreateObject("ADODB.Recordset")
	rs1.Open SQL1, REDconnstring
	set rs1 = nothing
...
Dim ProjID
	...
	LoopNum = CStr(Request.QueryString("LoopNum"))
						...
						SQL = "SELECT * FROM PurchaseNotes WHERE ProjID = "&ProjID
						set rs=Server.CreateObject("ADODB.Recordset")
						rs.Open SQL, REDconnstring	
				...
				TList = TList+("&lt;div class='NoteBox' id='NoteBox"&LoopNum&"'&gt;")
						...
						TList = TList+(" onkeyup='UpdateNotesandToDo(""PurchaseNotes"",""Note"",value,"""&rs("ID")&""")' ")
...
Dim ProjID
	...
	ToDoNum = 1
						...
						SQL = "SELECT * FROM PurchaseToDo WHERE ProjID = "&ProjID
						set rs=Server.CreateObject("ADODB.Recordset")
						rs.Open SQL, REDconnstring	
				...
				if rs("Selected") = "True" then Selected = "checked='checked'" end if		
					...
					TList = TList+("onclick='SelectNotesAndToDo("""&rs("ID")&""",""PurchaseToDo"","""&ProjID&""",""ToDoBoxInside"&LoopNum&""","""&rs("Selected")&""")'")
					...
					TList = TList+("onkeyup='UpdateNotesandToDo(""PurchaseToDo"",""ToDo"",value,"""&rs("ID")&""")' ")
...
Dim DivID
	...
	DivID = CStr(Request.QueryString("DivID"))
						...
						SQL = "UPDATE PurchaseToDo SET Checked = '"&TrueOrFalse&"'  WHERE ID = "&ToDoID
						set rs=Server.CreateObject("ADODB.Recordset")
						rs.Open SQL, REDconnstring	
