...
function NewEmpList()
{
	...
	if(!!JobsList.document.getElementById('JobName'+SelectedRow))
	{
...
function NewItemList()//--------------------------------------------------------
{
	...
	if(JobName.replace(" ","")==""){ alert("Please name this Task"); return false }

	
	if(!confirm('Creating the List: '+JobName/*+'.\n'+EmpID+'\n  ParentID='+ParentIDEmpID=*/)){return false}
...
function AddNewJob()
{
	...
	SQL='INSERT INTO JobsLists (Type, Date, Job, Attn, Active) VALUES ('+TaskType+', \''+now+'\', \'NEW JOB\', \''+parent.accessEmpName+'\', 1 )';
	SendSQL('Write',SQL);  
	JobsList.location=JobsList.location;
}
