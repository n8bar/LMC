...
function saveNewSec(section) {
	HttpText='BidASP.asp?action=saveNewSec&section='+CharsEncode(section)+'&projId='+projId
	xhr=getXHRObject();
		...
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				...
				try	{	var xmlDoc = xhr.responseXML.documentElement;	}
					...
					AjaxErr('There was a problem with the SaveNewSec response.',HttpText);	
				}
...
function AddPart(PartID) {
	var secId = Gebi("SecPage").contentWindow.secId;

	HttpText='BidASP.asp?action=AddPart&PartID='+PartID+'&SecID='+secId;
	xhr = getXHRObject();
...
function AddBlankPart() {
	var secId = Gebi("SecPage").contentWindow.secId;
	HttpText='BidASP.asp?action=AddPart&PartID=0&SecID='+secId;
	xhr = getXHRObject();
	xhr.onreadystatechange = ReturnAddPart;
	xhr.open('Get',HttpText, true);
	xhr.send(null);
...
function ReturnAddPart() {
	if (xhr.readyState == 4) {
		if (xhr.status == 200) {
			...
			var SecID = xmlDoc.getElementsByTagName("SecID")[0].childNodes[0].nodeValue;
...
function AddLabor(LaborID) {
	...
	HttpText='BidASP.asp?action=AddLabor&LaborID='+LaborID+'&SecID='+secId;
	xhr = getXHRObject();
...
function AddBlankLabor() {
	var secId = Gebi("SecPage").contentWindow.secId;
	HttpText='BidASP.asp?action=AddLabor&SecID='+secId;
	xhr = getXHRObject();
	xhr.onreadystatechange = ReturnAddLabor
	xhr.open('Get',HttpText, true);
	xhr.send(null);
...
function AddExpense(Type,SubType,Origin,Dest,Units,Cost) {
	var secId = Gebi("SecPage").contentWindow.secId;
	//var MU = Gebi('SecExpFrame').contentWindow.document.getElementById("MU").value;
	
	//alert(secId);
	SubType=CharsEncode(SubType);
	Origin=CharsEncode(Origin);
	Dest=CharsEncode(Dest);
	HttpText='BidASP.asp?action=AddExpense&Type='+Type+'&SubType='+SubType+'&Origin='+Origin+'&Dest='+Dest+'&Units='+Units+'&Cost='+Cost+'&SecID='+secId;
	xhr = getXHRObject();
	xhr.onreadystatechange = function () {
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
...
function PartsList(secId) {
	HttpText='BidASP.asp?Action=PartsList&SecID='+Gebi("SecPage").contentWindow.secId;
	xhr=getXHRObject();
	xhr.onreadystatechange=function() {
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				...
				var M=1+(MU/100);
				var partListDiv=Gebi('SecPage').contentWindow.document.getElementById('BidSectionsParts')
				...
				for(r=1;r<=recordCount;r++)	{
					...
					Manufacturer=CharsDecode(xmlDoc.getElementsByTagName('Manufacturer'+r)[0].childNodes[0].nodeValue.replace('--',''));
					ItemName=CharsDecode(xmlDoc.getElementsByTagName('ItemName'+r)[0].childNodes[0].nodeValue.replace('--',''));
				...
				Gebi('SecPage').contentWindow.sectionCost(secId);

				Gebi('SecPage').contentWindow.Gebi('TotalParts').innerHTML=formatCurrency(partsTotal);
			}
			else {
...
function LaborList(SecID) {
	HttpText='BidASP.asp?Action=LaborList&SecID='+Gebi("SecPage").contentWindow.secId;
	xhr=getXHRObject();
	xhr.onreadystatechange=function() {
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				...
				var MU=(xmlDoc.getElementsByTagName("MU")[0].childNodes[0].nodeValue*1);
				var M=1+(MU/100);
				var laborListDiv=Gebi('SecPage').contentWindow.document.getElementById('BidSectionsLabor')
				...
				for(r=1;r<=recordCount;r++)	{
					...
					if (!costing) {
						...
						Gebi('SecPage').contentWindow.sectionCost(Gebi('SecPage').contentWindow.secId);
						
						Gebi('SecPage').contentWindow.Gebi('TotalLabor').innerHTML=formatCurrency(laborTotal);
					}
				}
			}
...
function TravelList(SecID) {
	HttpText='BidASP.asp?Action=ExpenseList&Type=Travel&SecID='+Gebi("SecPage").contentWindow.secId;
	xhr=getXHRObject();
	xhr.onreadystatechange=function() {
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				...
				var travelListDiv=Gebi('SecPage').contentWindow.document.getElementById('BidSectionsTravel')
				...
				for(r=1;r<=recordCount;r++)	{
					...
					if(!costing) {
						...
						rowHTML+='	<div style="width:10%;" class=taRPi id=Travel-Cost'+tRow+' onKeyUp=updateTravelRow('+tRow+'); onblur="evalBox(this); updateTravelRow('+tRow+');">'+formatCurrency(Cost)+'</div>';
						rowHTML+='	<div style="width:4%;" class=taRPi id=Travel-Qty'+tRow+' onKeyUp=updateTravelRow('+tRow+'); onblur="evalBox(this); updateTravelRow('+tRow+');">'+Qty+'</div>';
						...
						Gebi('SecPage').contentWindow.Gebi('TotalTravel').innerHTML=formatCurrency(travelTotal);
					}
				}
			}
			else {
...
function EquipList(SecID) {
	HttpText='BidASP.asp?Action=ExpenseList&Type=Equip&SecID='+Gebi("SecPage").contentWindow.secId;
	xhr=getXHRObject();
	xhr.onreadystatechange=function() {
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				...
				var equipListDiv=Gebi('SecPage').contentWindow.document.getElementById('BidSectionsEquipment')
				...
				for(r=1;r<=recordCount;r++)	{
					...
					if(!costing) {
						...
						rowHTML+='	<div style="width:8%;" class=taRPi id=Equip-Qty'+eRow+' onKeyUp=updateEquipRow('+eRow+'); onblur="evalBox(this); updateEquipRow('+eRow+');">'+Qty+'</div>';
						rowHTML+='	<div style="width:40%;" class=taLPi id=Equip-Desc'+eRow+'>'+SubType+'</div>';
						rowHTML+='	<div style="width:20%;" class=taRPi id=Equip-Cost'+eRow+' onKeyUp=updateEquipRow('+eRow+');  onblur="evalBox(this); updateEquipRow('+eRow+');">'+formatCurrency(Cost)+'</div>';
						...
						Gebi('SecPage').contentWindow.Gebi('TotalEquip').innerHTML=formatCurrency(equipTotal);
					}
				}
			}
			else {
...
function OtherList(SecID) {
	HttpText='BidASP.asp?Action=ExpenseList&Type=OH&SecID='+Gebi("SecPage").contentWindow.secId;
	xhr=getXHRObject();
	xhr.onreadystatechange=function() {
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				...
				var otherListDiv=Gebi('SecPage').contentWindow.document.getElementById('BidSectionsOther')
				...
				for(r=1;r<=recordCount;r++)	{
					...
					if(!costing) {
						...
						rowHTML+='	<div style="width:8%;" class=taRPi id=Other-Qty'+oRow+' onKeyUp=updateOtherRow('+oRow+'); onblur="evalBox(this); updateOtherRow('+oRow+');">'+Qty+'</div>';
						rowHTML+='	<div style="width:40%;" class=taLPi id=Other-Desc'+oRow+'>'+SubType+'</div>';
						rowHTML+='	<div style="width:20%;" class=taRPi id=Other-Cost'+oRow+' onKeyUp=updateOtherRow('+oRow+'); onblur="evalBox(this); updateOtherRow('+oRow+');">'+formatCurrency(Cost)+'</div>';
						...
						Gebi('SecPage').contentWindow.Gebi('TotalOther').innerHTML=formatCurrency(otherTotal);
					}
				}
			}
			else {
	...
	xhr.onreadystatechange = function() {
		if(xhr.readyState==4) {
			if(xhr.status==200) {
				...
				//AjaxErr(' There was a problem with the sectionCost response.', HttpText)
				...
				if(useNewBidder)	{
					...
					if(totalFixed) { //Fixed Total 2010 Formula
						...
						var secTotal=(PL*M)+(parts*taxRate)+EO;
						var tax=parts*taxRate;
					...
					function p(number) {
						return Math.round((number/secTotal)*10000)/100
					}
	...
	xhr.onreadystatechange = function() {
		if(xhr.readyState==4) {
			if(xhr.status==200) {
				...
				var partsSell;
				var laborSell;
				function secCost(totalFixed,roundUp,fixedTotal,overheadRate,parts,labor,expenses,MU,taxRate) {
					...
					if(useNewBidder)	{
						if(totalFixed) { //Fixed Total 2010 Formula
							...
							tax=parts*taxRate;
							profit=secTotal-PLO-expenses-tax;
							MU=Math.round((M-1)*100000)/1000;
						}
						else { //Fixed Margin 2010 Formula
							overhead=overheadRate*(parts+labor+expenses);
							var PLO=parts+labor+overhead; var M=1+(MU/100);
							secTotal=(PLO*M)+(parts*taxRate)+expenses;
							tax=parts*taxRate;
						...
						if(totalFixed) { //Fixed Total Old Formula
							...
							profit=secTotal-PL-EO-tax;
							MU=Math.round((M-1)*100000)/1000;
						}
						else { //Fixed Margin Old Formula
							overhead=overheadRate*(parts+labor+expenses);
							var PL=parts+labor; var EO=expenses+overhead; var M=1+(MU/100);
							secTotal=(PL*M)+((parts*M)*taxRate)+EO;
							tax=(((parts)*M)+parts)*taxRate;
				...
				//secCost(totalFixed,fixedTotal,overheadRate,parts,labor,expenses,MU,taxRate)
				...
				for(r=1;r<=recordCount;r++) {
					...
					projTotal+=secCost(totalFixed,roundUp,fixedTotal,overheadRate,parts,labor,expenses,MU,taxRate);
					pTotal+=parts;
					lTotal+=labor;
					eTotal+=expenses;
					oTotal+=overhead;
