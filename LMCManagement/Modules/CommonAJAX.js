...
function Validate() {
	...
	xmlHttp.onreadystatechange = function(){ try{if(xmlHttp.status == 200){ReturnValidate();}}catch(e){} };
...
function ReturnValidate() {
	...
	if (xmlHttp.readyState == 4)	{ 
		if (xmlHttp.status == 200)	{
			...
			//alert(Validated);
			if(Validated!=1)	{
				//window.location="http://"+window.location.href.substr(6)+"Website/LMC/LMC.Html";
				window.location="../LMC/LMC.Html";
...
function ReturnCalLogin()
{
	...
	{ 
		if (xmlHttp.status == 200)
		{
			...
			//	var token = google.accounts.user.login(scope);
			//	/** /
			//	returnUrl="https://www.rcstri.com/website/LMCManagement/LMC.html";
...
{	
	Gebi('LoadInfo').innerHTML='Sections';
	
	xmlHttp = GetXmlHttpObject();
	xmlHttp.onreadystatechange = ReturnGetSectionsList;
	HttpText='CommonASP.asp?action=GetSectionsList';
	xmlHttp.open('GET',HttpText, true);
	xmlHttp.send(null);
	//if(xmlHttp.status == 200){ReturnGetSectionsList();}
}

function ReturnGetSectionsList()
{
	if (xmlHttp.readyState == 4)
	{
		if (xmlHttp.status == 200)
		{
			...
			{
				AjaxErr('There is a problem with "GetSectionsList:'+Var+'" Data.',HttpText);
			}
			
			var Num = 1;
			
			SecListArray = new Array();
			
			SecListArray[0] = new Array('','');
			
			
			for (i = 1; i <= ListLength; i++)
			{
				var sSectionID = 'SectionID'+Num;
				var sSectionName = 'SectionName'+Num;					
				
				var SectionID = xmlDoc.getElementsByTagName(sSectionID)[0].childNodes[0].nodeValue;
				var SectionName = xmlDoc.getElementsByTagName(sSectionName)[0].childNodes[0].nodeValue;						
				
				SecListArray[i] = new Array(SectionID,SectionName);
				
				Num ++;
			}
			
			SectionsListArray = SecListArray;
		...
		{
			AjaxErr('There was a problem with the GetSectionsList request.',HttpText);
		}
	}
}
////-------------------------------------------------------------------------------------------------
