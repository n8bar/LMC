...
<head>
	...
	<link rel=stylesheet href="../TMCDevelopment/Library/CSS_DEFAULTS.css" media=all>
	
	<script type="text/javascript" src="../TMCDevelopment/Modules/rcstri.js"></script>
	<script type="text/javascript" src="../TMCDevelopment/Modules/Num2word.js"></script>
	...
	PrintChecked = rs4("PrintChecked")
		...
		Do Until rs6.EOF
			...
			If rs("pParts")="True" Then
				...
				boxStyle="style=""overflow:hidden; text-overflow:ellipsis; white-space:nowrap; """
				%>
				<div class="w80p taC fs1-8" style="height:.1875in; margin-left:10%; overflow:hidden;">
					<div id=Qty<%=bIId%> class="h100p w10p fL taRP" <%=boxStyle%>><font face=Consolas><%=qty%></font></div>
					<div id=PN<%=bIId%> class="h100p w20p fL taLP" <%=boxStyle%>><%=DecodeChars(rs6("ItemName"))%></div>
					<%
					If PartsPrice Then
						%>
						<div id=Desc<%=bIId%> class="h100p w40p fL taLP" <%=boxStyle%>><%=DecodeChars(rs6("ItemDescription"))%></div>
						<div id=Price<%=bIId%> class="h100p w15p fL taRP" <%=boxStyle%>><font face=Consolas><%=formatCurrency(itemPrice)%></font></div>
						<div id=Total<%=bIId%> class="h100p w15p fL taRP" <%=boxStyle%>><font face=Consolas><%=formatCurrency(itemTotal)%></font></div>
						<%
					Else
						%><div id=Desc<%=bIId%> class="h100p w70p fL taLP" <%=boxStyle%>><%=DecodeChars(rs6("ItemDescription"))%></div><%
					End If
				%>
				</div>
				<%
		...
		If rs("pPartsTotal") = "True" Then
			...
			<div class="w80p taR" style="height:.25in; margin-left:10%; overflow:hidden;">
				Total: &nbsp;<font face=Consolas><%=formatCurrency(parts)%></font>
			</div>
			<%
		End If
		...
		If rs("pLabor")="True" Then
			...
			<div class="w80p taC" style="border-bottom:.01in solid #ccc; height:.1875in; margin-left:10%;">
				...
				If LaborPrice Then
					%>
					<div class="w50p fL taC" <%=boxStyle%> >Description</div>
					<div class="w20p fL taC">$</div>
					<%
				Else
					%><div class="w70p fL taC"> <%=boxStyle%>Description</div><%
				End If
				%>
			</div>
			<%
		...
		Do Until rs7.EOF
			...
			If rs("pLabor")="True" Then
				...
				<div class="w80p taC fs1-8" style="height:.1875in; margin-left:10%; overflow:hidden;">
					<div id=Qty<%=bIId%> class="h100p w10p fL taRP" <%=boxStyle%>><font face=Consolas><%=qty%></font></div>
					<div id=LN<%=bIId%> class="h100p w20p fL taLP" <%=boxStyle%>><%=DecodeChars(rs7("ItemName"))%></div>
					<%
					If LaborPrice Then
						%>
						<div id=Desc<%=bIId%> class="h100p w50p fL taLP" <%=boxStyle%>><%=DecodeChars(rs7("ItemDescription"))%></div>
						<div id=Qty<%=bIId%> class="h100p w20p fL taRP" <%=boxStyle%>><font face=Consolas><%=formatCurrency(itemTotal)%></font></div>
						<%
					Else
						%><div id=Desc<%=bIId%> class="h100p w70p fL taLP" <%=boxStyle%>><%=DecodeChars(rs7("ItemDescription"))%></div><%
					End If
				%>
				</div>
				<%
		...
		If rs("pLaborTotal") = "True" Then
			%>
			<div class="w80p taR" style="height:.25in; margin-left:10%; overflow:hidden;">
				Total: &nbsp;<font face=Consolas><%=formatCurrency(labor)%></font>
			</div>
			<br/>
			<%
		End If
...
<div class=w100p>
	<div class="w50p taC fL">
		...
		<font class="w80p taC fs3-32" face="Calibri" style='display:block; margin-left:10%; height:.125in;'><%=rs("pSignedTCS")%></font>
	</div>
	<div class="w50p taC fL">
		<%=Customer%> Representative
		<div class=w80p style="border-bottom:.01in #000 solid; margin-left:10%; height:.375in;"></div>
		<font class="w80p taC fs3-32" face="Calibri" style='display:block; margin-left:10%; height:.125in;'><%=rs("pSignedCust")%></font>
	</div>
</div>
<hr class="fL w100p"/>
