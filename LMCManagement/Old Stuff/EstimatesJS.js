...
function rebuildOldBidder(jsString)
{
	if(!PGebi('oldBidderFrame'))
	{
		parent.document.body.innerHTML+='<iframe id=oldBidderFrame frameborder=0 src="OldEstimates.html"></iframe>'
	}
	if(!!jsString)	{	setTimeout(jsString,1000);	}
}
...
function ProjTodoToggle()
{
	...
	if(Gebi('ProjView').checked==true)
	{
		...
		PGebi('oldBidderFrame').style.visibility='visible';	// We use the visibility style because we're using the display toggle in the OpenProject function.
	...
	{
		Gebi('ProjectContainer').style.display='block';
		PGebi('oldBidderFrame').style.visibility='hidden';
...
//	if(IEver>0&&(NewWidth<0||NewHeight<0)){return false}
	...
	Gebi('RightContainer').style.width = '100%';//Math.abs(DocW-Gebi('LeftContainer').offsetWidth-2)+'px';
...
function ShowTabCBids()
{
	...
	Gebi("AreaTab").style.background = '#9FD26C';
	Gebi("CustTab").style.background = '#9FD26C';
	Gebi("CBidsTab").style.background = '#40631D';
...
function ShowTabCust()
{
	...
	Gebi("CustTab").style.background = '#40631D';
	Gebi("AreaTab").style.background = '#9FD26C';
	Gebi("CBidsTab").style.background = '#9FD26C';
...
function ShowTabArea()
{
	...
	Gebi("CustTab").style.background = '#9FD26C';
	Gebi("AreaTab").style.background = '#40631D';
	Gebi("CBidsTab").style.background = '#9FD26C';
...
function ShowTabLeftCosting()
{
	...
	Gebi("InfoLeftTab").style.background = '#9FD26C';
	Gebi("CostingLeftTab").style.background = '#40631D';
...
function ShowSysTab(TabID)
{
	...
	Gebi("InfoTab").style.background = '#70AB34';
	Gebi("AltsTab").style.background = '#70AB34';
	Gebi("PartsTab").style.background = '#70AB34';
	Gebi("LaborTab").style.background = '#70AB34';
	Gebi("ExpensesTab").style.background = '#70AB34';

	Gebi(TabID.replace('Tab','Box')).style.display = 'block';
	Gebi(TabID).style.background = '#40631D';
...
//alert(ProjTabArray);	
	...
	for(i in ProjTabArray)
	{
	  ...
	  Gebi(ProjTabArray[i]).style.background = '#70AB34';
	}
	
	Gebi(Tab).style.background = '#40631D';	
...
function ShowProjMainTab()
{
	...
	Gebi("ProjPrintTab").style.background = '#70AB34';
...
function ShowProjPrintTab()
{
	...
	Gebi("ProjMainTab").style.background = '#70AB34';
	Gebi("ProjPrintTab").style.background = '#40631D';
...
function OpenPrintMain()
{
	...
	var HTTP = 'EstimatePrintMain.asp?ProjID='+ProjID+'';
...
function OpenPrintPartsLabor(PartsOrLabor)
{
	var ProjID = Gebi('HiddenProjID').value;
	var ShowCosts;
	if(Gebi('ckPrintCost').checked){ShowCosts=1;}
	else{ShowCosts=0;}
	var HTTP = 'EstimatePrintPartsLabor.asp?ProjID='+ProjID+'&PartsOrLabor='+PartsOrLabor+'&Costs='+ShowCosts;
	var openWindow = new Array;
