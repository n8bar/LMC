...
function ReturnMakeInfoBox()
{
	if (xmlHttp.readyState == 4)
	{
		if (xmlHttp.status == 200)
		{
			...
			HTML+='		<img class="CustomIX" src="../../TMCDevelopment/images/closesmall.gif" onClick="DelInfoBox('+InfoBoxID+',this.parentNode.parentNode)"/>';
...
function ReturnMakeInfoBoxData()
{
	if (xmlHttp.readyState == 4)
	{
		if (xmlHttp.status == 200)
		{
			...
			HTML+='	<img src="../../TMCDevelopment/images/closesmall.gif" style="cursor:pointer;" onClick="DelInfoBoxData('+InfoBoxDataID+',this.parentNode);"/>';
	...
	if (xmlHttp.readyState == 4)
	{
		if (xmlHttp.status == 200)
		{
			function xmlData(tagName)
			{
				...
				{
					Gebi('PartsResults').innerHTML+='<br/>Cannot Find XML tag:<font face="consolas">'+tagName+'</font>   <a href="../../TMCDevelopment/'+HttpText+'" target="_blank">?</a>';
				}
			}
