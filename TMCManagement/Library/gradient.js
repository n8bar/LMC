...
function createGradient() {
	...
	///
	// Ummmm I ran into wierd funny shtufs happenin' with browsers other than IE, hence the folowing line.
	if(IEver!=7&&IEver!=8&&IEver!=9){return false}
	...
	for(i=0;i<objArray.length;i++) {
		...
		if(document.all && !window.opera) {
			...
			for(p=0;p<colorArray.length;p++) {
				...
				g.setAttribute("style","position:static;z-index:0;top:" + y + "px;left:" + x + "px;height:" + h + "px;width:" + w + "px;background-color:rgb(" + colorArray[p][0] + "," + colorArray[p][1] + "," + colorArray[p][2] + ");");
...
function makeGrandParent(obj) {
	...
	mHTML = obj.innerHTML;
	obj.innerHTML = "";
	nSpan.innerHTML = mHTML;						//relative
	nSpan.setAttribute("style","position:static;z-index:10;");
