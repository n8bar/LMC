...
function UpdateText(BoxID,BoxType,Table,IDColumn,Column,RowID)
{
	...
	//alert(Text);
	HttpText = 'EstimatesASP.asp?action=UpdateText&Text='+CharsEncode(Text)+'&Table='+Table+'&IDColumn='+IDColumn+'&Column='+Column+'&RowID='+RowID+'&SysOK='+SysOK+'&BoxID='+BoxID
...
function UpdateSystemPrint(SysID)
{
	  ...
	  xmlHttp.open('Get','EstimatesASP.asp?action=UpdateSystemPrint&SysID='+SysID+'&PrintChecked='+PrintChecked, true);
	  xmlHttp.send(null);
}
...
function ToggleFT(Checked)
{
	...
	var txtProfit=PGebi('SysExpFrame').contentWindow.document.getElementById('MU');
	var txtFixedTotal=PGebi('SysExpFrame').contentWindow.document.getElementById('FixedTotal');
	
	txtProfit.readOnly=Checked;
	txtFixedTotal.readOnly=!Checked;
	
	SendSQL('Write','Update Systems SET FixedPrice='+txtFixedTotal.value+', MU='+txtProfit.value.replace('%','')+' WHERE SystemID='+SysID)
	
	if(Checked)
	{
		//txtFixedTotal.focus();
		txtProfit.style.fontFamily='Arial, Helvetica, sans-serif';
		txtProfit.style.fontWeight='bold';
		txtProfit.style.fontSize='12px';
		//txtProfit.style.color='#1F300E'; 
		txtFixedTotal.style.fontFamily='Consolas, "Courier New", Courier, monospace';
		txtFixedTotal.style.fontWeight='normal';
		txtFixedTotal.style.fontSize='13px';
		//txtFixedTotal.style.color='#000'; 
	}
	else
	{
		//txtProfit.focus();
		txtFixedTotal.style.fontFamily='Arial, Helvetica, sans-serif';
		txtFixedTotal.style.fontWeight='bold';
		txtFixedTotal.style.fontSize='12px';
		//txtFixedTotal.style.color='#1F300E'; 
		txtFixedTotal.style.textShadow='-1px -1px 1px #FFF,-1px -1px 1px #FFF';
		txtProfit.style.fontFamily='Consolas, "Courier New", Courier, monospace';
		txtProfit.style.fontWeight='normal';
		txtProfit.style.fontSize='13px';
		//txtProfit.style.color='#000';
		PGebi('SysInfoFrame').contentWindow.CalculateEstTotal('NoToggleFT');
	}
...
function GetAreaList()
{
	...
	xmlHttp.open('Get','EstimatesASP.asp?action=GetAreaList', true);
	xmlHttp.send(null);
}
...
function LoadAreaProj()
{
	...
	//alert(AreaID);
	HttpText='EstimatesASP.asp?action=LoadAreaProj&AreaID='+AreaID+'';
	xmlHttp = GetXmlHttpObject();
...
function GetCustomer(CustText)
{
	...
	xmlHttp.open('Get','EstimatesASP.asp?action=FindCust&CustText='+CustText+'', true);
	xmlHttp.send(null);
}
...
function GetProjects()
{
	...
	HttpText=encodeURI('EstimatesASP.asp?action=FindProj&CustID='+SearchCustID+'&Area='+SearchArea+'&DateFrom='+DateFrom+'&DateTo='+DateTo+'&Obtained='+Obtained);
		...
		if (xmlHttp.status == 200)
		{
			...
			if(ProjCount>0)
			{
				...
				for(P=1;P<=ProjCount;P++)
				{
					...
					rebuildOldBidder();
					onClick='"parent.document.getElementById(\'oldBidderFrame\').contentWindow.OpenProject('+pID+');"';
					textShadow='';
					Color='#000';
					if(Use2010Bidder=='True')
					{
						...
						textShadow='text-shadow:#70AB34 1px 1px 1px';
					}
					
					ProjListHTML+='<div id="Proj'+pID+'" class="CustListBox" style="height:auto;">'
					
					if(pID==0)
					{
						ProjListHTML+=	'<div onclick="alert(\'Please refine the search.\');" Title="'+pName+'"';
						ProjListHTML+=	' style="font-weight:normal; color:#800; font-family:Georgia; font-size:12px; height:auto;">'+pName+'</div>';
						continue;
					}
					
					ProjListHTML+='<a href="javascript:Void();" onclick='+onClick+' class="ProjListItems" style="color:'+Color+'; '+textShadow+'" Title="'+pName+'">'+pName+'</a>'
					ProjListHTML+='</div>'
				}
			}
...
{
	rebuildOldBidder
	PGebi("oldBidderFrame").style.display="none";
	
	Gebi("ProjectOverlay").style.display = 'block';
	Gebi('ProjectOverlayTxt').innerHTML = 'Opening Project';
	
	HttpText='EstimatesASP.asp?action=OpenProject&ProjID='+ProjID
	xmlHttp = GetXmlHttpObject();
...
/**/	
	...
	function NewProject()
	{
		...
		xmlHttp.open('Get','EstimatesASP.asp?action=NewProject&ProjName='+ProjName+'&CustID='+CustID+'', true);
		xmlHttp.send(null);
	}
...
//	var ProjCustID = Gebi('NewProjCustID').value;
	...
	*/
	...
	HttpText = HttpText +'EstimatesASP.asp?action=NewProjSave&ProjName=' + ProjName;
...
function UnBidTo(CustName)
{
	...
	xmlHttp.open('Get','EstimatesASP.asp?action=UnBidTo&CustName='+CustName+'&ProjID='+HiddenProjID.value, true);
	xmlHttp.send(null);
}
...
function UnBidToAll()
{
	...
	xmlHttp = GetXmlHttpObject();
	xmlHttp.onreadystatechange = ReturnUnBidTo;
	xmlHttp.open("Get","EstimatesASP.asp?action=UnBidTo&CustName=--ALL--&ProjID="+HiddenProjID.value, true);
	xmlHttp.send(null);
}
...
function AddBidToCust(CustomerID,CustName,Contact)
{
	...
	//alert(CustomerID);
	HttpText =encodeURI('EstimatesASP.asp?action=AddBidToCust&ProjID='+HiddenProjID.value+'&Contact='+Contact+'&ProjName='+CharsEncode(ProjName)+'&CustID='+CustomerID+'&CustName='+CustName);
	//DebugBox(a(Http));
...
function ObtainBid()
{
	...
	xmlHttp.open('Get','EstimatesASP.asp?action=ObtainBid&ProjID='+ProjID+'&ProjName='+ProjName+'&CustName='+CustName+'&CustID='+CustID, true);
	xmlHttp.send(null);
}
...
function Contract(isSigned)
{
	...
	{
		if(!confirm('Under normal circumstances you shouldn\'t edit a contracted system! \n\n Press Ok to edit it anyway or press Cancel to leave it alone. \n\n Note:When there are no contracted systems left, the entire project becomes de-activated.'))
		{
...
function UnContract()
{
	...
	xmlHttp.open('Get','EstimatesASP.asp?action=UnContract&ProjID='+ProjID, true);
	xmlHttp.send(null);
}
...
function GetBidTo()
{
	...
	xmlHttp.open('Get','EstimatesASP.asp?action=GetBidTo&ProjID='+HiddenProjID.value, true);
	xmlHttp.send(null);
}
...
function NewSystem()
{
		...
		HttpText='EstimatesASP.asp?action=NewSystem&NewSysEnteredBy='+NewSysEnteredBy+'&NewSysName='+NewSysName+'&NewSysDate='+NewSysDate+'&NewSysMU='+NewSysMU+'&NewSysTax='+NewSysTax+'&ProjID='+ProjID+'&CustID='+CustID+'';
		xmlHttp = GetXmlHttpObject();
...
{
	SelectedSysID=SysID;
	HttpText='EstimatesASP.asp?action=OpenSystem&SysID='+SysID;
...
function ReturnGetSystem() 
{
	if (xmlHttp.readyState == 4)
	{
		if (xmlHttp.status == 200)
		{
			...
			Gebi('SysInfoFrame').src='EstimatesSysInfo.asp?SysID='+SysID
			Gebi('SysExpFrame').src='EstimatesSysExpenses.asp?SysID='+SysID
			Gebi('SysAltsFrame').src='EstSysAlts.asp?SysID='+SysID
		}
		else
		{
...
{
	HttpText='EstimatesASP.asp?action=PartsList&SysID='+SysID;
	xmlHttp = GetXmlHttpObject();
...
function ReturnPartsList()
{
	if (xmlHttp.readyState == 4)
	{
		if (xmlHttp.status == 200)
		{
			...
			var Parts = xmlDoc.getElementsByTagName('Parts')[0].childNodes[0].nodeValue.replace('--','');
...
{
	HttpText='EstimatesASP.asp?action=LaborList&SysID='+SysID;
	xmlHttp = GetXmlHttpObject();
...
function ToggleActiveProject(ProjID,Active)
{
	...
	if (answer)
	{
		...
		xmlHttp.open('Get','EstimatesASP.asp?action=ToggleActiveProject&ProjID='+ProjID+'&Active='+Active, true);
		xmlHttp.send(null);
	}
	else
	{
...
function CheckBoxSysUpdate(SysID)
{
	  ...
	  if (CheckedOrNot == false){Checked = 0; var Text = 'You have chosen to Place this Project into INACTIVE status, press ok to continue';}
		...
		{
			xmlHttp = GetXmlHttpObject();
			xmlHttp.open('Get','EstimatesASP.asp?action=CheckBoxSysUpdate&SysID='+SysID+'&Checked='+Checked, true);
			xmlHttp.send(null);
		}
		else
		{
...
function UpdateSystemInfo(ObjectName,SQLName,Type)
{
	  ...
	  xmlHttp = GetXmlHttpObject();
	  xmlHttp.open('Get','EstimatesASP.asp?action=UpdateSystem&SysID='+SysID+'&SQLName='+SQLName+'&ObjValue='+ObjValue+'', true);
	  xmlHttp.send(null);
}
...
function UpdateAllItemCosting(SysID)
{
	  ...
	  xmlHttp.open('Get','EstimatesASP.asp?action=UpdateAllItemCosting&SysID='+SysID, true);
	  xmlHttp.send(null);
}
...
/*	  
	...
	HttpText='EstimatesASP.asp?action=ListEntryUpdate&RowID='+RowID+'&FieldName='+Field+'&TextString='+TextString;
	xmlHttp = GetXmlHttpObject();
...
function CalculateItemRow(RowID)
{
	...
	Gebi(CostSub).value = Math.round(sCostSub*Math.pow(10,2))/Math.pow(10,2);//rounds the decimal to 2 places
	
	Gebi('SysInfoFrame').contentWindow.CalculateEstTotal();
	/*
	HttpText='EstimatesASP.asp?action=ListSubItemsUpdate&CostSub='+sCostSub+'&RowID='+RowID;
	xmlHttp = GetXmlHttpObject();
 ...
 if (xmlHttp.readyState == 4)
	{
		if (xmlHttp.status == 200)
		{
			...
			{
				var xmlDoc = xmlHttp.responseXML.documentElement;
				var RowID = xmlDoc.getElementsByTagName("RowID")[0].childNodes[0].nodeValue.replace('--','');
			}
			catch(e)
			{
				AjaxErr('There was a problem with the CalculateItemRow response.',HttpText)
			}
			//alert(RowID);
...
{
	HttpText='EstimatesASP.asp?action=CalcProjTotals&ProjID='+Gebi('HiddenProjID').value;
	xmlHttp = GetXmlHttpObject();
...
function ReturnCalcProjTotals()
{
	if (xmlHttp.readyState == 4)
	{
		if (xmlHttp.status == 200)
		{
			...
			//AjaxErr('View the CalcProjTotals response?',HttpText);
			/**/
			...
			var Tax= xmlDoc.getElementsByTagName('Tax')[0].childNodes[0].nodeValue*1;
			var Overhead= xmlDoc.getElementsByTagName('Overhead')[0].childNodes[0].nodeValue*1;
			var Profit= xmlDoc.getElementsByTagName('ProfitDollars')[0].childNodes[0].nodeValue*1;
			...
			PartsCost=formatCurrency((PartsCost*100)/100);
			
			var LaborCostPercent=(Math.round(LaborCost/TotalPrice*10000)/100);
			LaborCost=formatCurrency((LaborCost*100)/100);
			
			var TravelPercent=(Math.round(Travel/TotalPrice*10000)/100);
			Travel=formatCurrency((Travel*100)/100);
			
			var EquipmentPercent=(Math.round(Equipment/TotalPrice*10000)/100);
			Equipment=formatCurrency((Equipment*100)/100);
			
			var OverheadPercent=(Math.round(Overhead/TotalPrice*10000)/100);
			Overhead=formatCurrency((Overhead*100)/100);
			
			var TaxPercent=(Math.round(Tax/TotalPrice*10000)/100);
			Tax=formatCurrency((Tax*100)/100);
			
			//var Profit=TotalPrice-(PartsCost+LaborCost+Travel+Equipment+Overhead+Tax);
			var ProfitPercent=(Math.round(Profit/TotalPrice*10000)/100);
			Profit=formatCurrency((Profit*100/100));
			...
			Gebi('ProjInfoTax').innerHTML=''+Tax+' <div style="float:right;">'+TaxPercent+'% </div>';			
...
function CalculateEstTotal(FTToggler)
{
	//return false;
	...
	HttpText='EstimatesASP.asp?action=CalculateEstTotal&SysID='+SelectedSysID+'&FixedTotal='+FixedTotal+'&FTToggler='+FTToggler;
	xmlHttp = GetXmlHttpObject();
...
function ReturnCalculateEstTotal() 
{
	if (xmlHttp.readyState == 4)
	{
		if (xmlHttp.status == 200)
		{
			...
			var Parts      = xmlDoc.getElementsByTagName("PartsCost")[0].childNodes[0].nodeValue*1;
			var Labor      = xmlDoc.getElementsByTagName("LaborCost")[0].childNodes[0].nodeValue*1;
			var Travel     = xmlDoc.getElementsByTagName("Travel")[0].childNodes[0].nodeValue*1;
			var Equipment  = xmlDoc.getElementsByTagName("Equipment")[0].childNodes[0].nodeValue*1;
			var TaxRate    = xmlDoc.getElementsByTagName("TaxRate")[0].childNodes[0].nodeValue*1;
			var Overhead   = xmlDoc.getElementsByTagName("Overhead")[0].childNodes[0].nodeValue*1;
			var Profit     = xmlDoc.getElementsByTagName("Profit")[0].childNodes[0].nodeValue*1;
			var FixedPrice = xmlDoc.getElementsByTagName("FixedPrice")[0].childNodes[0].nodeValue*1;
			...
			var FTToggler = xmlDoc.getElementsByTagName("FTToggler")[0].childNodes[0].nodeValue.replace('--','');
			...
			var Expenses=(Travel+Equipment)*1;
			var TaxCost=Math.round(TaxRate*Parts)/100;
			//var OverheadCost=(Overhead*((Parts*1)+(Labor*1)+(Expenses*1)))///100;
			...
			if(TotalFixed=='True')
			{
				...
				TotalSell=(FixedPrice*1)
				var OverheadCost=Math.round(Overhead*FixedPrice)/100;
				
				var ProfitDollars=FixedPrice-OverheadCost-Expenses-Parts-Labor-TaxCost
			...
			else
			{
				...
				var OverheadCost=Math.round(Overhead*FixedPrice)/100;
				TotalSell=FixedPrice+OverheadCost;
				var ProfitDollars=Math.round(Profit*TotalSell)/100;
				TotalSell+=TaxCost
				
				if(Round=='True')
				{
					var withProfit=TotalSell+ProfitDollars;
					var roundUp10= (Math.round(withProfit/10)*10)+10;
					var roundDiff= roundUp10-withProfit;
					ProfitDollars+=roundDiff;
				}
				
				TotalSell+=ProfitDollars;
				
				FixedPrice=TotalSell;
			}
			
			PGebi('SysInfoFrame').contentWindow.Gebi('PartsSell').innerHTML = '&nbsp;$'+Math.round(Parts*100)/100;
			PGebi('SysInfoFrame').contentWindow.Gebi('LaborSell').innerHTML = '&nbsp;$'+Math.round(Labor*100)/100;
			PGebi('SysInfoFrame').contentWindow.Gebi('Travel').innerHTML = '&nbsp;$'+Math.round(Travel*100)/100;
			PGebi('SysInfoFrame').contentWindow.Gebi('Equipment').innerHTML = '&nbsp;$'+Math.round(Equipment*100)/100;
			PGebi('SysInfoFrame').contentWindow.Gebi('OH').innerHTML = '&nbsp;$'+OverheadCost;
			PGebi('SysInfoFrame').contentWindow.Gebi('Profit').innerHTML = '&nbsp;'+formatCurrency(ProfitDollars);
			PGebi('SysInfoFrame').contentWindow.Gebi('Subtotal').innerHTML = '&nbsp;'+formatCurrency(TotalSell-TaxCost);
			PGebi('SysInfoFrame').contentWindow.Gebi('Tax').innerHTML = '&nbsp;$'+TaxCost;
			
			
			PGebi('SysInfoFrame').contentWindow.Gebi('TotalSell').innerHTML = formatCurrency(TotalSell);
			PGebi('SysTotal').innerHTML=formatCurrency(TotalSell);
			
			UpdateFixedTotal(FixedPrice);
			
			var ProfitBox=PGebi('SysExpFrame').contentWindow.document.getElementById('MU');
			var OldProfitBoxReadonly=ProfitBox.readonly;
			ProfitBox.readonly=false;
			ProfitBox.value=Math.round(Profit);
			ProfitBox.readonly=OldProfitBoxReadonly;
			/*
			...
			if(SqFtAdd=='True'){PGebi('SysInfoFrame').contentWindow.Gebi('ckIncludeSqFt').checked=true;}
			else{Gebi('ckIncludeSqFt').checked=false;}

			if(Round=='True'){PGebi('SysInfoFrame').contentWindow.document.getElementById('ckRound').checked=true;}
			else{Gebi('ckRound').checked=false;}

			if(ExcludeSys=='True'){PGebi('SysInfoFrame').contentWindow.Gebi('ckIncludeSys').checked=false;}
			else{Gebi('ckIncludeSys').checked=true;}

			PGebi('SysInfoFrame').contentWindow.Gebi('SysSqFt').value = SqFt;
			if(SqFt==null||SqFt==''||SqFt==0||isNaN(SqFt))
			{
				PGebi('SysInfoFrame').contentWindow.Gebi('SysSqFt').value = 0;
				SqFt=1;
			}
			
			var SqFtPrice=formatCurrency((TotalSell*1)/SqFt);
			
			Gebi('SysSqFtPrice').innerHTML = '&nbsp;'+SqFtPrice;
			
			var PartsPercent=Math.round((Parts/FixedPrice)*10000)/100;
			var LaborPercent=Math.round((Labor/FixedPrice)*10000)/100;
			var TravelPercent=Math.round((Travel/FixedPrice)*10000)/100;
			var EquipmentPercent=Math.round((Equipment/FixedPrice)*10000)/100;
			var OverheadCostPercent=Math.round((OverheadCost/FixedPrice)*10000)/100;
			var TaxRatePercent=(Math.round(TaxRate*10000)/10000);
			
			if(isNaN(PartsPercent))PartsPercent='&nbsp; 0';
			if(isNaN(LaborPercent))LaborPercent='&nbsp; 0';
			if(isNaN(TravelPercent))TravelPercent='&nbsp; 0';
			if(isNaN(EquipmentPercent))EquipmentPercent='&nbsp; 0';
			if(isNaN(OverheadCostPercent))OverheadCostPercent='&nbsp; 0';
			if(isNaN(TaxRatePercent))TaxRatePercent='&nbsp; 0';
			
			Gebi('SysInfoParts').innerHTML='&nbsp;'+PartsPercent+('%');
			Gebi('SysInfoLabor').innerHTML='&nbsp;'+LaborPercent+('%');
			Gebi('SysInfoTravel').innerHTML='&nbsp;'+TravelPercent+('%');
			Gebi('SysInfoEquipment').innerHTML='&nbsp;'+EquipmentPercent+('%');
			Gebi('SysInfoOverhead').innerHTML='&nbsp;'+OverheadCostPercent+('%');
			Gebi('SysInfoTax').innerHTML=''+TaxRatePercent+'% <span style="font-weight:normal; font-size:10px;"> of Materials</span>';
			Gebi('SysInfoTotal').innerHTML='100%';
			
			//if((TaxCost*1)>0){Gebi('SysInfoTotal').innerHTML+='<span style="font-weight:normal; font-size:12px;">&nbsp;+tax</span>'}
			
			var Profit=(TotalSell-Expenses)/TotalSell;
			
			var ProfitPercent=Math.round((ProfitDollars/FixedPrice)*10000)/100;
			if(isNaN(ProfitPercent))ProfitPercent='&nbsp; 0';
			
			Gebi('SysInfoProfit').innerHTML='&nbsp;'+ProfitPercent+('%');
			...
			var GraphData='';
			if(Parts>0)GraphData+='Materials-'+Math.round(Parts*100)+'-C6882D';
			if(Labor>0)
			{
				if(GraphData!='')GraphData+='.';
				GraphData+='Labor-'+Math.round(Labor*100)+'-';
			}
			if(Travel>0)
			{
				if(GraphData!='')GraphData+='.';
				GraphData+='Travel-'+Math.round(Travel*100)+'-';
			}
			if(Equipment>0)
			{
				if(GraphData!='')GraphData+='.';
				GraphData+='Equipment-'+Math.round(Equipment*100)+'-';
			}
			if(OverheadCost>0)
			{
				if(GraphData!='')GraphData+='.';
				GraphData+='Overhead-'+Math.round(OverheadCost*100)+'-';
			}
			if(ProfitDollars>0)
			{
				if(GraphData!='')GraphData+='.';
				GraphData+='Profit-'+Math.round(ProfitDollars*100)+'-';
			}
			
			
			PGebi('SysInfoFrame').contentWindow.document.getElementById('GraphFrame').src='pie.asp?GraphData='+GraphData;
			
			if(FTToggler!='NoToggleFT')
			{
				PGebi('SysInfoFrame').contentWindow.ToggleFT(PGebi('SysExpFrame').contentWindow.document.getElementById('ckFixedTotal').checked);			
			}
...
var FixedTotalBoxTries=0;
function UpdateFixedTotal(FixedPrice)
{
	var FixedTotalBox=parent.document.getElementById('SysExpFrame').contentWindow.document.getElementById('FixedTotal');
	try
	{
		var OldFixedTotalBoxReadonly=FixedTotalBox.readonly;
		FixedTotalBox.readonly=false;
		FixedTotalBox.value=formatCurrency(FixedPrice).replace(/,/g,'');
		FixedTotalBox.readonly=OldFixedTotalBoxReadonly;
		FixedTotalBoxTries=0;
	}
	catch(e)
	{
		if(FixedTotalBoxTries>15)
		{
			alert('Error: Cannot update "Fixed Price" box under Expenses.');
			return false;
		}
		setTimeout('UpdateFixedTotal('+FixedPrice+');',250);
		FixedTotalBoxTries++
	}
}
//-------------------------------------------------------------------------------------------------
...
function ItemSelected(ItemID,CBID)
{
	//alert(ItemID);
	  ...
	  xmlHttp = GetXmlHttpObject();
	  xmlHttp.open('Get','EstimatesASP.asp?action=ItemSelected&ItemID='+ItemID+'&ItemChecked='+ItemChecked, true);
	  xmlHttp.send(null);
}
//-------------------------------------------------------------------------------------------------
...
{
	  xmlHttp = GetXmlHttpObject();
	  xmlHttp.open('Get','EstimatesASP.asp?action=UncheckAll', true);
	  xmlHttp.send(null);
}
...
function DeleteItems(List)
{
	  ...
	  xmlHttp.open('Get','EstimatesASP.asp?action=DeleteItems&SysID='+SysID+'&List='+List, true);
	  xmlHttp.send(null);
}
...
function DeleteProject(ProjID)
{
	...
	if(confirm('REALLY!!!!?????'))
	{
		...
		xmlHttp.open('Get','EstimatesASP.asp?action=DeleteProject&ProjID='+ProjID+'&AreaIndex='+AreaIndex, true);
		xmlHttp.send(null);
	}
}
...
function DeleteSystem()
{
	...
	{ 
		...
		if (answer2)
		{
			HttpText='EstimatesASP.asp?action=DeleteSystem&SysID='+SysID;
			xmlHttp = GetXmlHttpObject();
...
function SearchLabor(SearchName)
{
	...
	HttpText='EstimatesASP.asp?action=SearchLabor&SearchTxt='+SearchTxt+'&SearchName='+SearchName;
	xmlHttp = GetXmlHttpObject();
...
function AddLabor(LaborID)
{
	...
	xmlHttp.open('Get','EstimatesASP.asp?action=AddLabor&LaborID='+LaborID+'&SysID='+SysID, true);
	xmlHttp.send(null);
}
function AddBlankLabor()
{
	...
	xmlHttp = GetXmlHttpObject();
	xmlHttp.onreadystatechange = ReturnAddLabor;
	xmlHttp.open('Get','EstimatesASP.asp?action=AddLabor&LaborID=0&SysID='+Gebi('HiddenEstID').value, true);
	xmlHttp.send(null);
}
...
function PrintSystemsList()
{
	...
	HttpText='EstimatesASP.asp?action=SystemsList&ProjID='+ProjID;
	xmlHttp = GetXmlHttpObject();
...
function LetterHeadSW(CB1,CB2,CB3)
{
	...
	xmlHttp = GetXmlHttpObject();
	xmlHttp.open('Get','EstimatesASP.asp?action=LetterHeadSW&ProjID='+ProjID+'&TFP_TCS='+TFP_TCS+'&TFP='+TFP+'&TCS='+TCS, true);
	xmlHttp.send(null);
...
function MakeLicenseFooters()
{
	...
	HttpText='EstimatesASP.asp?action=MakeLicenseFooters&HTML='+CharsEncode(FooterHTML)+'&ProjID='+Gebi('HiddenProjID').value;
	xmlHttp = GetXmlHttpObject();
...
function BidPresetList()
{
	  ...
	  xmlHttp.open('Get','EstimatesASP.asp?action=BidPresetList&SystemID='+SystemID, true);
	  xmlHttp.send(null);
...
function BidPresetCreate()
{
	...
	var MU = PGebi('SysExpFrame').contentWindow.document.getElementById('MU').value;
	
	HttpText='EstimatesASP.asp?action=BidPresetCreate&PresetID='+PresetID+'&SystemID='+SysID+'&MU='+MU;
	xmlHttp = GetXmlHttpObject();
...
function ReturnBidPresetCreate() 
{
	if (xmlHttp.readyState == 4)
	{
		if (xmlHttp.status == 200)
		{
			//AjaxErr('See the BidPresetCreate response?',HttpText);
