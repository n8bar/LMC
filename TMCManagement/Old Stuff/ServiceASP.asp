...
Sub LoadNotes  () '------------------------------------------------------------------------
	...
	SQL1 ="SELECT * FROM JobsLists WHERE Type=2 ORDER BY "&OrderBy
	set rs1=Server.CreateObject("ADODB.Recordset")
	rs1.Open SQL1, REDconnstring
	...
	Dim ItemBgImage : ItemBgImage = "<div style='width:100%; height:1px; z-index:0;'><img src='images/RowBGBlue3.gif' width='100%' height='27px' /></div>"
	Do While Not rs1.EOF
...
'		List = List & ItemBgImage
		...
		End If
		List = List	& "				<div title='Set priority of task' id='Pri"&Length&"' class='ItemPri' onMouseOver='MouseOver(this,^White^);' onmouseout='MouseOut(this);'"	
...
Sub SaveTask  () '--------------------------------------------------------------------------------------------------------
	...
	SQL ="INSERT INTO JobsLists (Type,Date,Job,Cust,Area,Attn,Notes,Active)"
	SQL=SQL+"VALUES (2,'"&d8&"','"&Job&"','"&Cust&"','"&Area&"','"&Attn&"','"&Notes&"','True')"
	set rs=Server.CreateObject("ADODB.Recordset")
