...
If CustID<>0 or Area<>"All" or DateFrom <> "1/1/1900" or CDate(DateTo) < Date or Obtained=1 Then 'Don't Show anything unless the criteria is sufficiently refined.
	...
	Do Until rs.EOF 
		...
		If pCount > MaxResults-1 Then
			pCount=pCount+1
			List=List&"<Use2010Bidder"&pCount&">--True</Use2010Bidder"&pCount&">"
	...
	If CustID<>0 Then
		...
		SQL="SELECT * FROM BidTo WHERE CustID="&CustID
				
		%><SQL><%=SQL%></SQL><%
		set rsBidTo=Server.CreateObject("ADODB.Recordset")
		rsBidTo.Open SQL, REDconnstring
		
		Do Until rsBidTo.EOF
						
			SQL="SELECT * FROM Projects WHERE ProjID="&rsBidTo("ProjID")'&" AND CustomerID!="&CustID
			
			'If Obtained=1 Then ShowObtained=" AND Obtained='True' AND " Else ShowObtained="" 
			'SQL=SQL&ShowObtained&" AND (DateEnt BETWEEN '"&DateFrom&"' AND '"&DateTo&"') "
			'If Area<>"All" Then SQL=SQL&"AND (Area = '"&Area&"') "
			...
			If Not rsProj.EOF Then
				...
				If pCount > MaxResults Then Exit Do
				
				ProjNotListed=True
				For p=1 to pAIndex
					if pIDArray(p)=rsProj("ProjID") Then 
						ProjNotListed=False
						Exit For
					End If
				Next
				
				If ProjNotListed Then
					pCount=pCount+1
					Active = rsProj("Active")
					Obtained = rsProj("Obtained") 'variable reused
					if rsProj("JobCompleted") = 5 Then Finished="True" Else Finished="False"
					
					pID=rsProj("ProjID")
					pName=rsProj("ProjName")
					
					pAIndex=pAIndex+1
					pIDArray(pAIndex)=pID
	
					List=List&"<refine>0</refine>"
					List=List&"<Use2010Bidder"&pCount&">--"&rsProj("Use2010Bidder")&"</Use2010Bidder"&pCount&">"
					List=List&"<ProjID"&pCount&">"&pID&"</ProjID"&pCount&">"
					List=List&"<ProjName"&pCount&">--"&pName&"</ProjName"&pCount&">"
					List=List&"<Active"&pCount&">"&Active&"</Active"&pCount&">"
					List=List&"<Obtained"&pCount&">"&Obtained&"</Obtained"&pCount&">"
		...
		pCount=pCount+1'Buffer
		List=List&"<refine>0</refine>"
		List=List&"<Use2010Bidder"&pCount&">--True</Use2010Bidder"&pCount&">"
		List=List&"<ProjID"&pCount&">0</ProjID"&pCount&">"
		List=List&"<ProjName"&pCount&">&amp;nbsp;</ProjName"&pCount&">"
		List=List&"<Active"&pCount&">1</Active"&pCount&">"
		List=List&"<Obtained"&pCount&">1</Obtained"&pCount&">"
		List=List&"<Finished"&pCount&">1</Finished"&pCount&">"
...
response.Write("<ProjCount>0"&pCount&"</ProjCount>"&XML&"<Sub>FindProj</Sub><SQL>"&SQL&"</SQL></root>")
...
Sub OpenProject  () '----------Gets the list of Systems-------------------------------------------------------
	...
	Use2010Bidder=rs3("Use2010Bidder")
	if (IsNull(Use2010Bidder)) Or Use2010Bidder = "" Then Use2010Bidder="false"
	...
	XMLMain = XMLMain + "<Use2010Bidder>"&Use2010Bidder&"</Use2010Bidder>"	
...
Do While Not rs1.EOF
	...
	PL= PL&"&lt;div class='PartsItemTxtBox' style='width:6%; border-left:0px; '&gt;"
	...
	PL= PL&"&lt;div class='PartsItemTxtBox' style='width:53%;'&gt;"
...
XMLlabor = ""
	...
	Do While Not rs2.EOF
		...
		XMLlabor = XMLlabor+("&lt;div class='PartsItemTxtBox' style='width:61%;'&gt;")
...
'	if Cost2 = "" then Cost2 = "NULL" end if
	...
	SQL1 ="INSERT INTO Projects (Use2010Bidder, ProjName, ProjAddress, ProjCity, ProjState, ProjZip, SqFoot, Floors, Area, Franchise, SubOf, DateEnt, Active)"'	)"'	
	SQL1 = SQL1 &" VALUES (1, '"&ProjName&"', '"&ProjAddress&"', '"&ProjCity&"', '"&ProjState&"', '"&ProjZip&"', "&SqFoot&", "&Floors&", '"&Area&"', '"&Franchise&"', '"&SubOf&"', '"&DateEnt&"', 'True')"'	)"'
	set rs1=Server.CreateObject("ADODB.Recordset")
	rs1.Open SQL1, REDconnstring
	set rs1 = nothing
...
Sub CalcProjTotals
	...
	Do While Not rs.EOF 
		...
		If rs("ExcludeSys") <> "True" Then
			...
			SQL1="SELECT * FROM BidItems WHERE SysID="&SysID
			Set rs1=Server.CreateObject("ADODB.RecordSet")
			rs1.Open SQL1, REDConnString
			...
			Do Until rs1.EOF
				...
				Select Case rs1("Type")
					...
					Case "Part"
						sPartsCost=sPartsCost+(itemCost*itemQty)
					
					Case "Labor"
						sLaborCost=sLaborCost+(itemCost*itemQty)
			...
			sTax=rs("TaxRate")*sPartsCost/100
			
			sProfit=rs("MU")
			If sProfit="" Or IsNull(sProfit) Then sProfit=0
			...
			sTravel=0
			Do Until rs1.EOF
				sTravel=sTravel+(rs1("Units")*rs1("UnitCost"))
				rs1.MoveNext
			Loop
			...
			If sTax="" Or IsNull(sTax) Then sTax=0
			sExpenses=sTravel+sEquipment
			
			TotalSoFar= sPartsCost+sLaborCost+sTravel+sEquipment
			
			If rs("TotalFixed")=True Then
				
				FixedPrice=rs("FixedPrice")
				if FixedPrice="" Or IsNull(FixedPrice) Then FixedPrice=0
				sTotalPrice=(FixedPrice*1)+(sTax*1)
				if sTotalPrice="" Or IsNull(sTotalPrice) Then sTotalPrice=0
				sOverhead=rs("Overhead"): if sOverhead="" Then sOverHead=0
				sOverhead= FixedPrice*sOverhead/100	
				if sOverhead="" Or IsNull(sOverhead) Then sOverhead=0
				
				sProfitDollars=FixedPrice-sOverhead-sExpenses-sPartsCost-sLaborCost
				
				if sProfitDollars="" Or IsNull(sProfitDollars) Then sProfitDollars=0
				sProfitDollars=Round(sProfitDollars*100)/100

			Else

				FixedPrice=sPartsCost+sLaborCost+sExpenses
				if FixedPrice="" Or IsNull(FixedPrice) Then FixedPrice=0
				sOverhead=rs("Overhead")
				if sOverhead="" Or IsNull(sOverhead) Then sOverhead=0
				sOverhead=FixedPrice*sOverhead/100
				sTotalPrice=FixedPrice+sOverhead
				sProfitDollars=(sProfit*sTotalPrice)/100
				if sProfitDollars="" Or IsNull(sProfitDollars) Then sProfitDollars=0
				
				If rs("Round")="True" Then
					withProfit=0'sTotalPrice+sProfitDollars+0
					'response.Write(withProfit)
					roundUp10= (Round(withProfit/10)*10)+10
					roundDiff= roundUp10-withProfit
					sProfitDollars=sProfitDollars+roundDiff
				End If
				
				sTotalPrice=sTotalPrice+sProfitDollars
				
				FixedPrice=sTotalPrice
				sTotalPrice=sTotalPrice+sTax
			...
			'sTotalPrice=(sPartsCost*sMarginPercent)+(sLaborCost*sMarginPercent)+sTravel+sEquipment+sOverhead+sTax
			If sTotalPrice="" Or IsNull(sTotalPrice) Then sTotalPrice=0
			
			If rs("Round")="True" Then sTotalPrice=(CInt(sTotalPrice/10)*10)+10
			...
			ProfitDollars=ProfitDollars+sProfitDollars
	...
	<root>
		...
		<ProfitDollars>0<%=ProfitDollars%></ProfitDollars>
...
Sub CalculateEstTotal() 'Gets the System Estimaate Totals----------------------------------------------------
	...
	SysID = CStr(Request.QueryString("SysID"))
	FTToggler=CStr(Request.QueryString("FTToggler"))
	...
	OH=rs1("Overhead")
	TotalFixed=rs1("TotalFixed") : if TotalFixed="" Or (isNull(TotalFixed)) Then TotalFixed="False"
	...
	Travel=0
	Do Until rs2.EOF
		Travel=Travel+(rs2("Units")*rs2("UnitCost"))
		rs2.MoveNext
	Loop
	...
	If Profit="" Or (IsNull(Profit)) Then Profit=0
	If FixedPrice="" Or (IsNull(FixedPrice)) Then TotalSell=0
...
'			
'			
'				SQL2 = "UPDATE BidItems SET CostSub = "&CostSub&" WHERE SysID = "&ItemID
'				set rs2=Server.CreateObject("ADODB.Recordset")
'				'rs2.Open SQL2, REDconnstring
'				set rs2 = nothing
'			
...
XMLparts = ""
  ...
  set rs=Server.CreateObject("ADODB.Recordset")
	...
	Do While Not rs.EOF
		...
		XMLparts=XMLparts&"&lt;button class='PartsListItemAdd' onClick='parent.AddPart("&rs("PartsID")&");'&gt; Add &lt;/button&gt;&lt;/div&gt;"
...
Sub SystemsList  () '----------Gets the list of Systems------& BidTo Customers----------------------------------------------
	...
	Do Until rs.EOF 
		...
		ID="cbCust"&CustCount
		CustomerList = CustomerList & "<input type=*checkbox* id=*"&ID&"*/>"
	...
	UncheckCustomers="<script type=""text/javascript"">"
	UncheckCustomers=UncheckCustomers&"function UncheckPrintCustomers(){ "
	UncheckCustomers=UncheckCustomers&	"for(c=1;c<=CustCount;c++){"
	UncheckCustomers=UncheckCustomers&		"Gebi('cbCust'+c).checked=false;"
	UncheckCustomers=UncheckCustomers&	"}"
	UncheckCustomers=UncheckCustomers&	""
	UncheckCustomers=UncheckCustomers&	""
	UncheckCustomers=UncheckCustomers&	""
	UncheckCustomers=UncheckCustomers&	""
	UncheckCustomers=UncheckCustomers&"}"
	UncheckCustomers=UncheckCustomers&"</script><br/>"
	
	CustomerList=UncheckCustomers&CustomerList
