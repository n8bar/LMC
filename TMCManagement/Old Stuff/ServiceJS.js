...
//  Edits A Task//////////////////////////////////////////////////////////////////////////////
function EditTask(ServiceID)
{
	txtJob.value = document.getElementById('sizeJob'+ServiceID).innerHTML;
	txtDate.value = document.getElementById('Date'+ServiceID).innerHTML;
	txtCust.value = document.getElementById('sizeCust'+ServiceID).innerHTML;
	HiddenServiceID.value = ServiceID;
	
	for(var i=0;i<selArea.length;i++)
	{
		if (selArea[i].innerText == document.getElementById('Area'+ServiceID).innerHTML)
		{	selArea.selectedIndex = i;}
	}
	for(var i=0;i<selAttn.length;i++)
	{
		if (selAttn[i].innerText == document.getElementById('Attn'+ServiceID).innerHTML)
		{	selAttn.selectedIndex = i;}
	}
	
	txtNotes.innerText = document.getElementById('HiddenNotes'+ServiceID).value;

	var Job = document.getElementById('sizeJob'+ServiceID).innerHTML;
	TaskBox('Edit: '+Job);
}
//--------------------------------------------------------------------------------------------
...
{
	HeadJob.style.color='#CCC';
	HeadJob.style.fontSize='12px';
	HeadCust.style.color='#CCC';
	HeadCust.style.fontSize='12px';
	HeadArea.style.color='#CCC';
	HeadArea.style.fontSize='12px';
	HeadAttn.style.color='#CCC';
	HeadAttn.style.fontSize='12px';
	HeadDate.style.color='#CCC';
	HeadDate.style.fontSize='12px';
	HeadPri.style.color='#CCC';
	HeadPri.style.fontSize='12px';
	
	switch(Order)
	{	case 'Job':
			document.getElementById('HeadJob').style.color='#FFF';
			document.getElementById('HeadJob').style.fontSize='14px';
			break;
		case 'Priority':
			document.getElementById('HeadPri').style.color='#FFF';
			document.getElementById('HeadPri').style.fontSize='14px';
			break;
		case 'Cust':
			document.getElementById('HeadCust').style.color='#FFF';
			document.getElementById('HeadCust').style.fontSize='14px';
			break;
		case 'Area':
			document.getElementById('HeadArea').style.color='#FFF';
			document.getElementById('HeadArea').style.fontSize='14px';
			break;
		case 'Attn':
			document.getElementById('HeadAttn').style.color='#FFF';
			document.getElementById('HeadAttn').style.fontSize='14px';
			break;
		default:
			document.getElementById('HeadDate').style.color='#FFF';
...
function ToCal(ServiceID)
{
	...
	Target.getElementById('EventTitleText').value = document.getElementById('sizeJob'+ServiceID).innerHTML;
	Target.getElementById('FromDateTxt').value = document.getElementById('Date'+ServiceID).innerHTML;
	Target.getElementById('ToDateTxt').value = document.getElementById('Date'+ServiceID).innerHTML;
	...
	{		
		//alert(i);
		if(AttnSel[i].innerText == document.getElementById('Attn'+ServiceID).innerHTML)
		{
	...
	{		
		//alert(i);
		if(TaskList[i].innerText != TaskList[i].innerText.replace('Service',''))
		{
	...
	{		
		//alert(i);
		if(AreaSel[i].innerText == document.getElementById('Area'+ServiceID).innerHTML)
		{
...
function showProgressMenu(DivID,ServiceID,ColumnID,X,Y)
{
	...
	//var i = 1
	{
		MenuItems +='<div class="ProgressItems" onclick="ProgressClick('+ServiceID+','+pArray[i][1]+',&#39;'+DivID+'&#39;,'
...
function showPriMenu(DivID,ServiceID)
{
	...
	//var i = 1
	{
		MenuItems +='<div class="ProgressItems" onclick="PriClick('+ServiceID+','+prArray[i][1]+',&#39;'+DivID+'&#39;,'
