...
Sub LoadNotes  () '------------------------------------------------------------------------
	...
	SQL1 ="SELECT * FROM JobsLists WHERE Type=3 ORDER BY "&OrderBy
	set rs1=Server.CreateObject("ADODB.Recordset")
	rs1.Open SQL1, REDconnstring
	...
	Dim ItemBgImage : ItemBgImage="<div style='width:100%; height:1px; z-index:0;'><img src='images/RowBGred3.gif' width='100%' height='27px' style='z-index:0;'/></div>"
	Do While Not rs1.EOF
...
'		List = List & ItemBgImage
		...
		End If
		List = List	& "				<div title='Set task priority.' id='Pri"&Length&"' class='ItemPri' onMouseOver='MouseOver(this,^blue^);' onmouseout='MouseOut(this);'"	
...
Sub SaveTask  () '--------------------------------------------------------------------------------------------------------
	...
	SQL ="Insert into JobsLists (Type,Date,Job,Cust,Area,Attn,Notes,Active) VALUES (3,'"&d8&"','"&Job&"','"&Cust&"','"&Area&"','"&Attn&"','"&Notes&"','True')"
	set rs=Server.CreateObject("ADODB.Recordset")
...
Sub ArchiveTask() '--------------------------------------------------------------------------------------------------------
	...
	set rs=Server.CreateObject("ADODB.Recordset")
	If Progress = 5 Then rs.Open SQL, REDconnstring
	set rs = nothing
		
	XML = "<Event>Updated</Event><SQL>"&SQL&"</SQL><Progress>"&Progress&"</Progress>"
	XML = "<root>"&XML&"</root>"
