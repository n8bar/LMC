...
Sub GetBidList  () '----------------------------------------------------------------------------------------------------
	...
	set rs=Server.CreateObject("ADODB.Recordset")
	rs.Open SQL, REDconnstring	
	'Dim MaxBidding : MaxBidding=10

	BidList=rs.GetRows()
	
	L=L&"<RecordCount>"&uBound(BidList,1)&"</RecordCount><FieldCount>"&uBound(BidList,2)-1&"</FieldCount>"
	For LoopNum = 0 to uBound(BidList,2)-1
		L=L&"<Row"&LoopNum&">"
		For FieldNum=0 to uBound(BidList,1)-1
			FVal=BidList(FieldNum,LoopNum)
...
Sub RefreshNotesAndToDoSelect() '-------------------------------------------------------------------------------------------
	...
	SQL = "UPDATE BidToDo SET Selected = '0'"
	set rs=Server.CreateObject("ADODB.Recordset")
	rs.Open SQL, REDconnstring
	set rs = nothing

	SQL1 = "UPDATE BidNotes SET Selected = '0'"
	set rs1=Server.CreateObject("ADODB.Recordset")
	rs1.Open SQL1, REDconnstring
	set rs1 = nothing
...
Dim ProjID
	...
	LoopNum = CStr(Request.QueryString("LoopNum"))
						...
						SQL = "SELECT * FROM BidNotes WHERE ProjID = "&ProjID
						set rs=Server.CreateObject("ADODB.Recordset")
						rs.Open SQL, REDconnstring	
				...
				TList = TList+("&lt;div class='NoteBox' id='NoteBox"&LoopNum&"'&gt;")
						...
						TList = TList+(" onkeyup='UpdateNotesandToDo(""BidNotes"",""Note"",value,"""&rs("ID")&""")' ")
...
Dim ProjID
	...
	ToDoNum = 1
						...
						SQL = "SELECT * FROM BidToDo WHERE ProjID = "&ProjID
						set rs=Server.CreateObject("ADODB.Recordset")
						rs.Open SQL, REDconnstring	
				...
				if rs("Selected") = "True" then Selected = "checked='checked'" end if		
					...
					TList = TList+("onclick='SelectNotesAndToDo("""&rs("ID")&""",""BidToDo"","""&ProjID&""",""ToDoBoxInside"&LoopNum&""","""&rs("Selected")&""")'")
					...
					TList = TList+("onkeyup='UpdateNotesandToDo(""BidToDo"",""ToDo"",value,"""&rs("ID")&""")' ")
...
Dim DivID
	...
	DivID = CStr(Request.QueryString("DivID"))
						...
						SQL = "UPDATE BidToDo SET Checked = '"&TrueOrFalse&"'  WHERE ID = "&ToDoID
						set rs=Server.CreateObject("ADODB.Recordset")
						rs.Open SQL, REDconnstring	
