...
Sub NewTreeListItem () '----------------------------------------------------------------------------------------------------
	...
	SQL=SQL&" VALUES ('"&JobName&"', "&EmpID&", "&ParentID&", '"&Notes&"', '"&JobTable&"', "&JobTableID&",1)"
...
Sub GetProjectList  () '----------------------------------------------------------------------------------------------------
	...
	set rs=Server.CreateObject("ADODB.Recordset")
	rs.Open SQL, REDconnstring	
	'Dim MaxProjects : MaxProjects=10

	ProjectList=rs.GetRows()
	
	L=L&"<RecordCount>"&uBound(ProjectList,1)&"</RecordCount><FieldCount>"&uBound(ProjectList,2)-1&"</FieldCount>"
	For LoopNum = 0 to uBound(ProjectList,2)-1
		L=L&"<Row"&LoopNum&">"
		For FieldNum=0 to uBound(ProjectList,1)-1
			FVal=ProjectList(FieldNum,LoopNum)
...
Sub RefreshNotesAndToDoSelect() '-------------------------------------------------------------------------------------------
	...
	SQL = "UPDATE ProjectToDo SET Selected = '0'"
	set rs=Server.CreateObject("ADODB.Recordset")
	rs.Open SQL, REDconnstring
	set rs = nothing

	SQL1 = "UPDATE ProjectNotes SET Selected = '0'"
	set rs1=Server.CreateObject("ADODB.Recordset")
	rs1.Open SQL1, REDconnstring
	set rs1 = nothing
...
Dim ProjID
	...
	LoopNum = CStr(Request.QueryString("LoopNum"))
						...
						SQL = "SELECT * FROM ProjectNotes WHERE ProjID = "&ProjID
						set rs=Server.CreateObject("ADODB.Recordset")
						rs.Open SQL, REDconnstring	
				...
				TList = TList+("&lt;div class='NoteBox' id='NoteBox"&LoopNum&"'&gt;")
						...
						TList = TList+(" onkeyup='UpdateNotesandToDo(""ProjectNotes"",""Note"",value,"""&rs("ID")&""")' ")
...
Dim ProjID
	...
	ToDoNum = 1
						...
						SQL = "SELECT * FROM ProjectToDo WHERE ProjID = "&ProjID
						set rs=Server.CreateObject("ADODB.Recordset")
						rs.Open SQL, REDconnstring	
				...
				if rs("Selected") = "True" then Selected = "checked='checked'" end if		
					...
					TList = TList+("onclick='SelectNotesAndToDo("""&rs("ID")&""",""ProjectToDo"","""&ProjID&""",""ToDoBoxInside"&LoopNum&""","""&rs("Selected")&""")'")
					...
					TList = TList+("onkeyup='UpdateNotesandToDo(""ProjectToDo"",""ToDo"",value,"""&rs("ID")&""")' ")
...
Dim DivID
	...
	DivID = CStr(Request.QueryString("DivID"))
						...
						SQL = "UPDATE ProjectToDo SET Checked = '"&TrueOrFalse&"'  WHERE ID = "&ToDoID
						set rs=Server.CreateObject("ADODB.Recordset")
						rs.Open SQL, REDconnstring	
