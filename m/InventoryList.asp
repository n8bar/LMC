...
#itemsHead:firstChild{border-top-left-radius:9px;}
#itemsHead:lastChild{border-top-right-radius:9px;}
...
.qtyAdd { line-height:30px; height:30px; width:25%; color:white; font-weight:bold; margin:0; padding:0; float:left; border:1px solid #999; border-radius:8px;
...
.qtyAdd:active {opacity:1;}
.qtySub { line-height:30px; height:30px; width:25%; color:white; font-weight:bold; margin:0; padding:0; float:left; border:1px solid #999; border-radius:8px;
 background:-moz-linear-gradient(top, rgba(255,112,112,.6667), /*rgba(0,192,192,.5) 50%,*/ rgba(144,64,64,.6667));
 background:-webkit-gradient(linear,0 0,0 100%, from(rgba(255,112,112,.6667)) /*,color-stop(.5, rgba(0,192,192,.5))*/, to(rgba(144,64,64,.6667)));
}
.qtySub:active {opacity:1;}
...
SQL ="SELECT PartsID, PartNumber, Inventory, InventoryLevel, Manufacturer, Model, Description, Cost+(Cost*Margin/100) AS Retail FROM Parts WHERE "&where&" ORDER BY "&SortBy
...
Function rsDC(fieldName) 
	rsDC=DecodeChars(rs(fieldName))
End Function
...
Do until rs.Eof Or lI > 4096
	...
	<div class=row id=row<%=lI%> value="<%=rs("PartsID")%>" >
		<div class=rowTop id=rb<%=lI%>>
			<div style="width:30%;"><b>Name:</b> <%=rsDC("Model")%></div>
			<div style="width:20%;"><b>Mfr:</b> <%=rsDC("Manufacturer")%></div>
			<div style="width:20%;"><%=rsDC("Description")%></div>
			<div style="width:30%;">
				<button id=qty<%=lI%>+100 class=qtyAdd onclick="inventoryAdjust(<%=lI%>,100);" >+100</button>
				<button id=qty<%=lI%>+1000 class=qtyAdd onclick="inventoryAdjust(<%=lI%>,1000);" >1000</button>
				<button id=qty<%=lI%>-100 class=qtySub onclick="inventoryAdjust(<%=lI%>,-100);">-100</button>
				<button id=qty<%=lI%>-1000 class=qtySub onclick="inventoryAdjust(<%=lI%>,-1000);">1000</button>
			</div>
		</div>
		<div id=PN<%=lI%> style="width:30%; <%=bS%> " ><%=rsDC("PartNumber")%></div>
		...
		<div style="width:30%;">
			<button id=qty<%=lI%>+1 class=qtyAdd onclick="inventoryAdjust(<%=lI%>,1);" >+1</button>
			<button id=qty<%=lI%>+10 class=qtyAdd onclick="inventoryAdjust(<%=lI%>,10);" >+10</button>
			<button id=qty<%=lI%>-1 class=qtySub onclick="inventoryAdjust(<%=lI%>,-1);">-1</button>
			<button id=qty<%=lI%>-10 class=qtySub onclick="inventoryAdjust(<%=lI%>,-10);">-10</button>
		</div>
	</div>
	<%
	Response.Flush()
