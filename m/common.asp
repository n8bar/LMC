...
Sub MfrOptionList(idPrefix)
  mSQL="SELECT ManufID, Name from Manufacturers ORDER BY Name"
	...
	Do Until mRS.EOF
    %><option id=<%=idPrefix&mRS("ManufID")%> value="<%=mRS("ManufID")%>"><%=DecodeChars(mRS("Name"))%></option><%
		mRS.MoveNext
	Loop
End Sub
...
Sub CategoryOptionList(idPrefix)
  catSQL="SELECT CategoryID, Category from Categories ORDER BY Category"
	...
	Do Until catRS.EOF
    %><option id=<%=idPrefix&catRS("CategoryID")%> value="<%=catRS("CategoryID")%>"><%=DecodeChars(catRS("Category"))%></option><%
		catRS.MoveNext
	Loop
End Sub
...
%>	.w<%=pdec%>p {width:<%=p%>% !important;} <%
	...
	%>.o<%=pdec%> {opacity:<%=(p/100)%> !important;} <%
	%><%=chr(13)&chr(10)%><% 
...
.w66p {width:66.66667% !important;}
.w66p {width:66.6666% !important;}
.w33p {width:33.3333% !important;}
.w34p {width:33.3334% !important;}
